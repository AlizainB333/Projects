...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(
        ...
        ##### YOUR CODE STARTS HERE #####
         ...
         self_attn = self.self_attn(query=x, key=x, value=x, key_padding_mask=self_attn_padding_mask)
         self_attn = self.dropout(self_attn)
         self_attn = self.self_attn_layer_norm(residual + self_attn)
         residual = x # Update Residual

         self_attn_sec_layer = self.activation_fn(self.fc1(self_attn_sec_layer))
         self_attn_sec_layer  = self.fc2(self_attn_sec_layer)
         self_attn_sec_layer = self.dropout(self_attn_sec_layer)
         x = self.final_layer_norm(residual + self_attn_sec_layer)
