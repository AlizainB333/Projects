...
def get_returns(rollout_buffer: utils.RolloutBuffer, discount_factor=0.95):
    ...
    returns = [] # initlize the returns list and return
    ep_ret = 0
    # reversedBuffer = reversed(rollout_buffer.rewards)   # Reverse Buffer


    for rollout in reversed(range(len(rollout_buffer.rewards))):

        if rollout_buffer.terminateds[rollout]:     # Check if terminate state and reset
            ep_ret = 0  

        # Update episode return with discounted reward
        ep_ret = rollout_buffer.rewards[rollout] + discount_factor * ep_ret
        returns.insert(0, [ep_ret])  
...
def get_value_net_loss(value_net: nn.Module,
    ...
    """
    # YOUR CODE HERE
    loss = value_net(observation)
    return nn.functional.mse_loss(loss,returns)
...
def get_vanilla_policy_gradient_loss(policy: nn.Module,
    ...
    """
    # YOUR CODE HERE
    action_logits = policy(observation) # Convert to nn mod obsv

    log_probs = torch.gather(nn.functional.log_softmax(action_logits, dim=1), dim=1, index=action) # Calc prob

    loss = -torch.mean(log_probs*return_or_advantage) # Calc loss
        ...
        for _ in range(training_epochs_per_rollout):
            ...
            for batch_start in range(0, rollout_buffer.final_size, minibatch_size):
                ...
                policy_gradient_kwargs = dict(
                    ...
                    value_net=value_net,
                    critic=None,
                    observation=rollout_buffer.observations[batch_start:batch_stop],
                    old_logits=rollout_buffer.old_logits[batch_start:batch_stop],
                    action=rollout_buffer.actions[batch_start:batch_stop],
                    return_or_advantage=advantages[batch_start:batch_stop] if get_advantages else returns[batch_start:batch_stop],
                    returns=returns[batch_start:batch_stop],
                    ppo_clip=ppo_clip
                )
                # END YOUR CODE
                optimizer.zero_grad()
