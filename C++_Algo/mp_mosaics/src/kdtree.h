...
class KDTree
{
  ...
  private:
     ...
     */
    KDTreeNode *NewNode(vector<Point<Dim>> points,int curdim);

    KDTreeNode *CopyNode(const KDTreeNode *copy);

    void Destroy(const KDTreeNode *erase);

    Point<Dim> NeighborHelper(KDTreeNode* curnode, Point<Dim> target, Point<Dim> curBestPoint, int curDim) const;
