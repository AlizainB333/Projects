...
#define KERNEL_SIZE 3  // Adjust as per your kernel size (K x K)
#define CHANNELS 3     // Adjust according to the number of input channels

// Declare the constant memory for the kernel (weight matrix)
__constant__ float d_kernel[KERNEL_SIZE * KERNEL_SIZE * CHANNELS];
    ...
    if (row < Channel * UnrollSize) {
        ...
        for(int p = 0; p < K; p++) {
            for(int q = 0; q < K; q++) {
                ...
                output[h_unroll * UnrollSize + w_unroll] = in_4d(currBatch, colPtr, h_out + p, w_out + q) * d_kernel[p * K + q];  // Multiply by kernel weight
...
__host__ void GPUInterface::conv_forward_gpu_prolog(const float *host_output, const float *host_input, const float *host_mask, float **device_output_ptr, float **device_input_ptr, float **device_mask_ptr, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    cudaMemcpyToSymbol(d_kernel, host_mask, Map_out * Channel * K * K * sizeof(float));
