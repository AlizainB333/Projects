...
__global__ void matrix_unrolling_kernel(const float *input, float *output,
    ...
    */
    ...
    if (row < Channel * UnrollSize) {
        ...
        // Define unrolling factor
        const int unrollK = 4;  // Factor to unroll

        for (int p = 0; p < K; p++) {
            // Unrolling the inner loop to process 4 elements 
            for (int q = 0; q < K; q += unrollK) {
                size_t w_unroll = (h_out * Width_out + w_out) + currBatch * Width_out * Height_out;
                size_t h_unroll = w_base + p * K + q;

                // Unrolling to handle 4 values 
                if (q + 0 < K) {
                    output[h_unroll * UnrollSize + w_unroll] = in_4d(currBatch, colPtr, h_out + p, w_out + q + 0);
                }
                if (q + 1 < K) {
                    output[(h_unroll + 1) * UnrollSize + w_unroll] = in_4d(currBatch, colPtr, h_out + p, w_out + q + 1);
                }
                if (q + 2 < K) {
                    output[(h_unroll + 2) * UnrollSize + w_unroll] = in_4d(currBatch, colPtr, h_out + p, w_out + q + 2);
                }
                if (q + 3 < K) {
                    output[(h_unroll + 3) * UnrollSize + w_unroll] = in_4d(currBatch, colPtr, h_out + p, w_out + q + 3);
                }
            }
        }
