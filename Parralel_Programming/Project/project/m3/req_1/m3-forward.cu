                                     ...
                                     int numCRows, int numCColumns)
{
    ...
    // Shared memory for storing tile C
    ...
    // Loop over the K dimension (shared dimension between A and B)
    for (int tileId = 0; tileId < (numAColumns - 1) / TILE_WIDTH + 1; tileId++) {
        int aCol = tileId * TILE_WIDTH + threadIdx.x;
        int bRow = tileId * TILE_WIDTH + threadIdx.y;

        // Load tiles into shared memory
        if (cRow < numARows && aCol < numAColumns) {
            tileA[threadIdx.y * TILE_WIDTH + threadIdx.x] = __float2half(A[(size_t)cRow * numAColumns + aCol]);
        } else {
            tileA[threadIdx.y * TILE_WIDTH + threadIdx.x] = 0;
        }

        if (bRow < numBRows && cCol < numBColumns) {
            tileB[threadIdx.y * TILE_WIDTH + threadIdx.x] = __float2half(B[(size_t)bRow * numBColumns + cCol]);
        } else {
            tileB[threadIdx.y * TILE_WIDTH + threadIdx.x] = 0;
        }

        __syncthreads();

        // Load shared memory tiles into WMMA fragments
    ...
    if (cRow < numCRows && cCol < numCColumns) {
        C[cRow * numCColumns + cCol] = tileC[threadIdx.y * TILE_WIDTH + threadIdx.x];
    }
}
